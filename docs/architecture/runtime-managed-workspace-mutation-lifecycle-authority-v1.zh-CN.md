...
| accepted successor 与 canonical head | workspace successor authority writer | generic RuntimeEvent writer 不能写 reserved workspace fact |
...
- canonical expected path set；
- execution profile digest；
- object format。

该结构只接受平台无关的 canonical Git path。durable mode 在 T1 前确定；如果 admission 不可用，必须在
T1 前拒绝。T1 后缺失 terminal proof、proof 畸形或 owner 异常一律 fail-stop，禁止 generic fallback。
...
2. 验证 successor 的 parent/base/path/profile 与 T1 完全一致；
3. 写入 outcome RuntimeEvent；
4. 写入 successor workspace fact；
5. 更新 disposable version/head projections；
6. 释放 durable reservation。
...
- Runtime 在一次有界 strict-JSON traversal 中复制并冻结结果；
...
- managed success/safe-discard/unsettled 都不能触发 generic `commitOutcome()`。
...
1. 在 T1 前取得并冻结真实 execution/candidate capability；
