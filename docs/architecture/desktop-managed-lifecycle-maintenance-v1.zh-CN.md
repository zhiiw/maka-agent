...
Desktop 的正常 Review 读取不能隐式变成写操作。accepted review 成功后，Desktop 另外发送一个显式 command；该 command 只调用 managed session 的 GC owner，并且只能处理当前 epoch 固定 `orphans/` 根下已过期的 restore orphan。
...
Linux、macOS 和 Windows 使用相同 tombstone 协议。Windows 文件占用导致删除失败时 tombstone 保留到下一轮；所有平台都不承诺硬件断电持久性，也不进行 Git object reachability GC。
