...
Write/Edit 只有在 T1 前取得与 managed workspace、canonical base head、operation、exact paths 和 execution
profile 绑定的 owner capability，才允许落下 `managed_mutation_v1` T1。T1 durable 后，Runtime 不得自行写
generic T2；只有 workspace owner 能报告以下三种结算：

1. `workspace_successor_committed`：M2.1 的成功 T2、successor 与 head 已经原子提交；
2. `safely_discarded`：owner 已证明副作用未被接受，并提交 canonical error outcome；
...
```text
Tool Runtime
  -> ManagedWorkspaceOwner.admit(operation, paths, profile)
...
任一步失败都发生在 T1 前：工具实现不运行，普通 preflight error 可返回给 provider。caller 传入的数据在
第一次异步操作前复制并校验；路径语法复用 Core 的平台无关 canonical validator。
...
`safely_discarded` 的 durable writer 和 candidate 证明由 M2.4 提供；M2.3b 只固定 Runtime 不得越权结算。

## 5. 生命周期与失败状态

| 场景 | 结果 |
|---|---|
| admission 前取消、handle/path/profile/head 无效 | 无 T1，标准 preflight error |
...
- owner admission 冻结 caller 数据，拒绝同 workspace 第二 lease；
...
- safely-discarded error outcome 只采用 durable T2；
