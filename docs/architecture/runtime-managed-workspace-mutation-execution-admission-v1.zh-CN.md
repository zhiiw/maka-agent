...
|---|---|---|---|
| T1 reservation kill/reopen | 历史 CI 已证明；重建分支待 CI | 历史 CI 已证明；重建分支待 CI | 本地定向通过；重建分支待 CI |
| 跨进程唯一 mutation reservation | 同上 | 同上 | 同上 |
...
本次平铺重建保留 `sqlite-runtime-crash` 与 `sqlite-recovery-concurrency` 的真实进程用例，但不从等待
Gitoxide 重建的 M2.2 携带旧 candidate inventory。最终发布 CI inventory 在 M2.2/M2.4 重建时统一登记。
...
- real-process kill-after-T1 与双进程 reservation 竞争已进入 focused suite；三平台严格 inventory 等待
  Gitoxide M2.2/M2.4 重建后统一登记。
