...
生产环境的执行所有权分为控制面与数据面。host 在两次无状态 worker 请求之间持有
per-file lock，并负责 SQLite T1/T2；prepare/inspect 是 workspace-bounded、size-bounded
的本地证据读取，真正改变文件的 temp write、fsync、replace 和 parent fsync 只在
filesystem worker 中执行。配置了 worker 后，正常执行与 recovery redo 都不得静默回退
到 host-local apply；worker crash、timeout、协议响应不确定或 apply 返回
`effect_unsettled` 时，operation 保持 `prepared`，交给 authoritative recovery 判定。
worker 的 one-call sandbox capability 只包含 target、由 operationId 确定性导出的 temp、
Windows before backup，以及用于 fsync 的 parent directory 本身，全部使用 exact scope；
不得为实现 atomic replace 而授予 parent subtree 写权限。

POSIX 的文件 identity 同时包含 content hash 与精确的 `mode & 0o7777`。内容相同但 mode
漂移时 park `prepared_file_mode_drifted`，不能误报 finalize。Windows 的 `stat.mode` 是
模拟值，首版只以 content hash 作恢复判定，不把 ACL 映射误当 POSIX mode；Windows replace
前还会创建 operation-scoped、已 fsync 的 `maka-before` 备份，以覆盖“目标已删除、temp
尚未安装”的 split-replace 窗口。备份必须与 durable before hash 一致才能用于 redo。

atomic replace 会更换 inode。首版因此拒绝 hard-linked target，避免静默破坏 link
语义；它保留 POSIX permission bits，但不承诺保留 ACL、xattr、文件 owner 或其他
inode-scoped metadata。当前实现无法可移植地发现全部这类 metadata；依赖这些语义的
workspace 必须由 host policy 停用 prepared mutation。后续应通过平台专用 metadata
carrier 扩展，而不是假装已经保留。
...
- carrier 只读取 source Run 的 workspace cwd，拒绝越界路径、符号链接、hard link 和非普通文件；POSIX 恢复对 durable before/expected-after hash+mode 做三态判定，Windows 对 hash 与可验证的 before backup 做三态判定；
- filesystem worker 拥有实际 mutation；host 持有跨 prepare、T1、worker apply 和 T2 的 per-file lock。配置 worker 后禁止 host-local apply fallback；
- replace 已尝试后的任意异常都转换为 effect-aware unsettled error，不提交 error T2；prepare/inspect/current read 均受 32 MiB hard limit 约束；
...
| F3W | Windows before backup fsync 后、split replace 中断 | target 为 before、after 或 missing+verified backup；只能 redo/finalize，不得丢失 before 证据 |
...
prepared_file_mode_mismatch_park_count
prepared_file_effect_unsettled_count
```
