...
## 第五十三检查点：长恢复回归的分阶段时间预算

先保留原 60 秒总期限，逐一记录 `backend-live-sequence` 的真实 child process 耗时。实际共 16 次串行启动，每次约 2.6–6.7 秒，所有退出码均符合预期，包括故意 crash 的 87/88/89/90 和候选篡改拒绝的 1；最后 retry 也完成。总耗时约 65 秒，最终被父测试总 deadline 取消。证据指向不断累积的串行验收工作量超出旧预算，而不是某一进程挂死；不因此声称产品性能不存在其他问题。

调整仅在测试 owner：保留同一真实持久化历史、全部子进程和断言，将后半段明确为两个串行子测试。continuation 来源/祖先/漂移阶段预算 30 秒；T1-only/candidate tamper/接受/精确重试阶段预算 40 秒；设置阶段预留 20 秒，总 deadline 从这些命名预算求和为 90 秒。单个子进程仍维持原 20 秒硬超时，无静默重试、无 skip、无生产改动。这里确实修订了总期限，但发生在阶段测量之后，且新增了比单一宽 deadline 更具体的阶段约束，不是未经定位直接放大超时。
