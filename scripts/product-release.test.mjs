...
test('Desktop packaging includes only the admitted Gitoxide helper artifact', () => {
  const resources = desktopBuilderConfig.extraResources.map(({ from, to }) => ({ from, to }));
  assert.ok(
    resources.some(
      ({ from, to }) => from === '.generated/gitoxide-helper/gitoxide' && to === 'gitoxide',
    ),
  );
  assert.ok(
    resources.some(
      ({ from, to }) =>
        from === '.generated/gitoxide-helper/gitoxide-helper.json' && to === 'gitoxide-helper.json',
    ),
  );
  assert.ok(
    resources.some(
