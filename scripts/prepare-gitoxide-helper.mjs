...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const outputRoot = join(repoRoot, 'apps', 'desktop', '.generated', 'gitoxide-helper');
  ...
  console.log(
    `[gitoxide-helper] prepared ${manifest.executableRelativePath} (${manifest.bytes} bytes, ${manifest.sha256})`,
  );
}
