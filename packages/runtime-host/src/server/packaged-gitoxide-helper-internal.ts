...
const packagedReleaseOwnerToken = Object.freeze({ kind: 'packaged_gitoxide_release_owner_v1' });
const MANIFEST_KEYS = [
  'arch',
  'bytes',
  'distributionReady',
...
function decodeManifest(input: unknown): PackagedGitoxideHelperManifestV1 {
  ...
  if (
    ...
    !isExactOperationList(value.supportedOperations) ||
...
function isExactOperationList(value: unknown): value is readonly GitoxideHelperOperationInternal[] {
  return (
    Array.isArray(value) &&
    value.length === GITOXIDE_HELPER_OPERATIONS_INTERNAL.length &&
    value.every((operation, index) => operation === GITOXIDE_HELPER_OPERATIONS_INTERNAL[index])
  );
}
