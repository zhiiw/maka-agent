...
function isRuntimeManagedWorkspaceMutation(
  ...
  const paths = value.expectedPaths;
  return (
    new Set(paths).size === paths.length &&
    paths.every((path) => isCanonicalManagedMutationPath(path))
  );
}

function isCanonicalManagedMutationPath(path: unknown): path is string {
  if (
    typeof path !== 'string' ||
    path.length === 0 ||
    path.length > 4096 ||
