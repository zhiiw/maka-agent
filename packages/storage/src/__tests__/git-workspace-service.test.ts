...
describe("Git workspace service", () => {
  ...
  test("imports only the clean source HEAD tree into an independent fixed-config workspace", async () => {
    ...
    assert.notEqual(
      normalize(binding.repositoryPath),
      normalize(sourceSnapshot.commonDir),
    );
    assert.equal(
    ...
    assert.equal(
      existsSync(join(binding.worktreePath, ".maka-workspace.json")),
      false,
    );
    assert.equal(
      await git(binding.worktreePath, "status", "--porcelain=v1"),
      "",
    );
    assert.equal(
      await gitBare(
    ...
    assert.equal(
      existsSync(join(binding.repositoryPath, "objects", "info", "alternates")),
      false,
    );
    await assert.rejects(
      gitBare(binding.repositoryPath, "cat-file", "-e", sourceSnapshot.head),
    );
  ...
  test("adopts the same binding across service restart and concurrent duplicate opens", async () => {
    ...
    assert.equal(
      (await restarted.inspectManagedWorkspace(adopted)).state,
      "ready",
    );
  });
  ...
  test("reopens an existing managed epoch after the source checkout advances", async () => {
    ...
    const firstService = await serviceAt(storageRoot);
    const created =
      await firstService.createManagedWorkspaceFromSource(request);

    await writeFile(
      join(sourceRoot, "tracked.txt"),
      "source advanced\n",
      "utf8",
    );
    ...
    assert.equal(
      (await restarted.inspectManagedWorkspace(reopened)).state,
      "ready",
    );
    assert.equal(
      await readFile(join(reopened.worktreePath, "tracked.txt"), "utf8"),
      "tracked\n",
    );
  });
  ...
  test("creates a second epoch with a different baseline in the same managed repository", async () => {
    ...
    const sourceRoot = await createEligibleSource(join(root, "source"));
    const service = await serviceAt(join(root, "storage"));
    const first = await service.createManagedWorkspaceFromSource(
      openRequest(sourceRoot),
    );
    ...
    const second =
      await service.createManagedWorkspaceFromSource(secondRequest);
    ...
    assert.equal(
      await readFile(join(first.worktreePath, "tracked.txt"), "utf8"),
      "tracked\n",
    );
    assert.equal(
    ...
    assert.equal(
      (await service.inspectManagedWorkspace(second)).state,
      "ready",
    );
  });
  ...
  test(
    "repairs a real-process crash after worktree materialization without accepting residue",
    {
      timeout: 60_000,
    },
    async () => {
      ...
      const child = spawn(
        ...
        [
          fileURLToPath(
            new URL(
              "./fixtures/git-workspace-service-crash-child.js",
              import.meta.url,
            ),
          ),
        ],
      ...
      try {
        ...
        assert.equal(
          (await service.inspectManagedWorkspace(binding)).state,
          "ready",
        );
        ...
        assert.ok(
          quarantineEntries.some((entry) => entry.startsWith("incomplete-")),
        );
        await assertSourceUnchanged(sourceRoot, sourceSnapshot);
      } finally {
  ...
  test("rejects nested attributes that could change fixed Git materialization", async () => {
    ...
    await writeFile(
      join(sourceRoot, "nested", ".gitattributes"),
      "*.txt filter=custom\n",
      "utf8",
    );
  ...
  test("detects external managed-worktree drift and quarantines it without touching source", async () => {
    ...
    await writeFile(
      join(binding.worktreePath, "tracked.txt"),
      "external writer\n",
      "utf8",
    );
    ...
    const quarantined = await service.quarantineManagedWorkspace(
      binding,
      "external_drift",
    );
...
async function createEligibleSource(sourceRoot: string): Promise<string> {
  ...
  await writeFile(
    join(sourceRoot, "ignored.env"),
    "SECRET=not-imported\n",
    "utf8",
  );
  await writeFile(
    join(sourceRoot, ".maka-workspace.json"),
    '{"host":"metadata"}\n',
    "utf8",
  );
...
function isWorkspaceError(code: string): (error: unknown) => boolean {
  return (error) =>
    error instanceof GitWorkspaceServiceError && error.code === code;
}
...
function waitForReady(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      cleanupListeners();
      reject(
        new Error(`Git workspace crash child did not become ready: ${stderr}`),
      );
    }, timeoutMs);
    ...
    const onExit = (code: number | null, signal: NodeJS.Signals | null) => {
      ...
      reject(
        new Error(
          `Git workspace crash child exited early: ${code}/${signal} ${stderr}`,
        ),
      );
    };
