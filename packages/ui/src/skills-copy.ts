...
const SKILLS_COPY = {
  zh: {
    ...
    context: { title: '上下文检查', summary: (discovered, advertised, omitted, shadowed) => `发现 ${discovered} · 已展示 ${advertised} · 预算省略 ${omitted} · 被覆盖 ${shadowed}`, scope: { project: '项目', workspace: '工作区', user: '用户', custom: '自定义' }, decision: { advertised: '已进入上下文', disabled: '已停用', invalid: '元数据无效', host_incompatible: '主机不兼容', shadowed: '被高优先级覆盖', budget: '因预算省略' } },
  ...
  en: {
    ...
    context: { title: 'Context inspector', summary: (discovered, advertised, omitted, shadowed) => `Discovered ${discovered} · advertised ${advertised} · budget omitted ${omitted} · shadowed ${shadowed}`, scope: { project: 'Project', workspace: 'Workspace', user: 'User', custom: 'Custom' }, decision: { advertised: 'In context', disabled: 'Disabled', invalid: 'Invalid metadata', host_incompatible: 'Host incompatible', shadowed: 'Shadowed', budget: 'Budget omitted' } },
